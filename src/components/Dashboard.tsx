...
export function Dashboard() {
  ...
  // Handle sorting
  const handleSort = useCallback((field: keyof Reference) => {
    if (sortField === field) {
  ...
  const groupedReferences = sortedReferences.reduce((groups, ref) => {
  ...
  // Status sort order
  const sortedStatuses = STATUS_ORDER.filter((status) =>
    ...
    <div className="dashboard-container">
      ...
      <header className="dashboard-header">
        ...
        <div className="header-right">
          <button className="add-reference-btn" disabled title="Coming in Plan 04">
            + Add Reference
          </button>
        </div>
      </header>
      ...
      {references.length === 0 && (
        <div className="empty-state">
          ...
          <p>Add your first reference to get started tracking projects and files.</p>
          <button className="add-reference-btn-primary" disabled>
      ...
      {references.length > 0 && (
        <div className="table-container">
          <table className="reference-table">
            <thead>
              <tr>
                <th className="col-name" onClick={() => handleSort("referenceName")}>
                  Name {sortField === "referenceName" && (sortDirection === "asc" ? "↑" : "↓")}
                </th>
            ...
            <tbody>
              {sortedStatuses.map((status) => (
                <>
                  <tr key={`header-${status}`} className="status-header-row">
                    <td colSpan={6}>
                      <div className="status-header">
                        ...
                        <span className="status-count">
                          {groupedReferences[status].length}
                        </span>
                      </div>
                    </td>
                  </tr>
                    ...
                    >
                      <td className="cell-name">
                        <div className="name-cell">
                          {ref.pinned && <span className="pin-indicator">📌</span>}
                          <span className="reference-name">{ref.referenceName}</span>
                        </div>
                      </td>
                      <td className="cell-path">
                        <span className="path-text" title={ref.absolutePath}>
                          {truncatePath(ref.absolutePath)}
                        </span>
                      </td>
                      <td className="cell-type">
                        <span className={`type-badge ${ref.type}`}>{ref.type}</span>
                      </td>
                      ...
                      <td className="cell-tags">
                        <div className="tags-container">
                          ...
                          {ref.tags.length > 3 && (
                            <span className="tag-more">+{ref.tags.length - 3}</span>
                          )}
                        </div>
                      </td>
                      <td className="cell-actions">
                        <div className="action-buttons">
                          <button
                            ...
                            onClick={(e) => handleOpenInTerminal(e, ref)}
                          >
                            ⌘
                          </button>
                          <button
                            ...
                            onClick={(e) => handleOpenInVSCode(e, ref)}
                          >
                            ⌥
                          </button>
                          <button
                            ...
                            onClick={(e) => handleRevealInFinder(e, ref)}
                          >
                            R
                          </button>
                        </div>
